...
// Optional page load fallback, but never relied on as the only exit.
window.addEventListener("load", () => {
    setTimeout(dismissLoader, 300);
});

// FAILSAFE: ensure loader disappears no later than 1.5s on all devices.
loaderFallback = setTimeout(dismissLoader, 1500);

window.addEventListener("DOMContentLoaded", () => {
    setTimeout(dismissLoader, 1200);
});

// If the page is already loaded before script execution, dismiss immediately.
if (document.readyState === "complete") {
    setTimeout(dismissLoader, 300);
}

// Extra DOMContentLoaded safety for script loaded after DOM ready.
